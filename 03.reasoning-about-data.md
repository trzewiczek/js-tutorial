...
To make conjunction easier to remember let's present its working in the special table:
...
> We put additional paranthesis just to make sure how JavaScript will read our logical expression. If we think of it, putting the paranthesis around our alternative would give us completely different meaning of the expresson: `bmw.numberOfTransactions >= 500 && (bmw.total > 100000 || bmw.total > 250000)`. It became weird, didn't it? Let's skip it and get back to the main narrative. 
...
//                                                       true
```
For Citroën and Toyota—as we see above—either one or the other criterion turns true which makes them major customers. 

To make alternative easier to remember let's present its working in the special table:

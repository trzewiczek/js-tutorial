...
First of all the name `page` (declared as `const`) directs us to an array. This means we want to treat all links from our app's view as a collection, not individual entities. How we represent each of them is to be discovered. For now we only know that each of them is a part of a bigger collection. Is there any specific order into this list? Is this collection complete? Is it coherent? We don't know yet. Do we care about it though? Not yet.
